#' @title creating Area data file (txt) from tiff image file and optionally distance map, binary image
#' @description \code{AxArea} create Area data file
#' @import EBImage
...
#' @param Export TRUE: exporting txt file of object size percentage
#' @param Binary TRUE: exporting binary image files
#' @param DistMap TRUE: exporting image files of distancemap
#' @return return the list of file names and % area of axons
#' @export
#' @examples
#' # AxArea("C:/Users/R", Binary = TRUE, DistMap = TRUE)


AxArea <- function(x, CD = FALSE, Export = TRUE, Binary = FALSE, DistMap = TRUE){
    Mainf <- x
    if (CD == TRUE){
      cdd <- getwd()
